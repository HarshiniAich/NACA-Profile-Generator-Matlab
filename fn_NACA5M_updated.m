function [Name,Coordinates] = fn_NACA5M(CL,Dist_Max_Camber,Reflex,Max_Thick,LE_Radius,Dist_Max_Thick)
...
% Sixt Digit : -
% Seventh Digit : Leading edge radius [3-9]
% Eighth Digit :Position of the maximum thickness in 1/10 of the chord [1-9]

% Example : Naca 43112-62
% Usage: [Name,Coordinates] = fn_NACA5M(4,3,1,12,6,2)
% 4= Optimum lift coefficient x 0.15 Cl=0.6
% 3 = Position of Maximum camber x 5 = 15 % of the chord
% 1 = Cambrure reflex (1)
% 12 = Maximum thickness 12%
% - Hyphen
% 6= Leading edge radius Normal = 6
% 2= Maximum thickness at 20% of the chord
%
...
if(LE_Radius<3 || LE_Radius>9)
    IpError = 1;
end
if(Dist_Max_Thick<1 || Dist_Max_Thick>9)
    IpError = 1;
end
if(IpError ==1)
    disp('Error in Input!!!');
...
Name = sprintf('%d%d%d%02d-%d%d',CL,Dist_Max_Camber,Reflex,Max_Thick,LE_Radius,Dist_Max_Thick);
...
%value corrected by Harshini Aich 9 Mar 2021
data{i1}  = sprintf('CL = %0.1f',1.5*CL/10); i1=i1+1;
...
%value corrected by Harshini Aich 9 Mar 2021
data{i1}  = sprintf('XMAXC = %1.2f',5*Dist_Max_Camber/10); i1=i1+1;
data{i1}  = sprintf('LEINDEX = %d',LE_Radius); i1=i1+1;
data{i1}  = sprintf('XMAXT = %0.1f',Dist_Max_Thick/10); i1=i1+1;
